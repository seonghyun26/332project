...
def generate_tuples_to_file(num_tuples: int, filename: str):
    subprocess.run([f'{HOME}/gensort/gensort', '-s', str(num_tuples), filename])
...
def validate_file(filename: str) -> Tuple[bool, str]:
    p = subprocess.run([f'{HOME}/gensort/valsort', filename], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    return p.returncode == 0, p.stdout.decode()

def _get_gensort():
    git_repo = 'https://github.com/petergeoghegan/gensort.git'
    os.system(f'git clone {git_repo} {HOME}/gensort && cd {HOME}/gensort && make all')
